...
export default function ProductGrid() {
    ...
    const handleBuyNow = (product: (typeof dummyProducts)[0]) => {
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="flex flex-col gap-6">
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-end justify-between mt-auto gap-2 sm:gap-3">
                       ...
                       <div className="flex gap-2 w-full sm:w-auto">
                         <button 
                           onClick={(e) => {
                             e.stopPropagation();
                             handleAddToCart(dummyProducts[0]);
                           }}
                           className="flex-1 sm:flex-none bg-[#ff6700] text-white px-3 py-2.5 md:px-4 md:py-3 rounded-lg md:rounded-[14px] font-bold text-sm md:text-[15px] hover:bg-[#ff6700]/90 shadow-lg shadow-orange-500/20 transition-all duration-300 flex items-center justify-center gap-2 whitespace-nowrap"
                         >
             ...
             <div className="w-full grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 mt-2">
                {dummyProducts.filter(p => !p.featured).map((product) => (
                  <div key={product.id} className="bg-white rounded-[2rem] hover:-translate-y-1 hover:shadow-xl shadow-gray-200/50 transition-all duration-300 flex flex-col items-center justify-between p-4 md:p-6 text-center cursor-pointer group border border-gray-100 relative">
                    {product.name.includes("Mijia") || product.name.includes("17") ? 
                      <span className="bg-yellow-100 border border-yellow-200 text-yellow-800 text-[10px] font-bold px-2 py-0.5 rounded-sm absolute top-3 left-3 md:top-4 md:left-4">Baru</span> : null
                    }
                    ...
                    <button
                      ...
                      className={`absolute top-3 right-3 md:top-4 md:right-4 p-2 md:p-2.5 rounded-full transition-all duration-300 flex items-center justify-center z-10 ${
                        addedNotification === product.id
                          ? 'bg-green-500 text-white scale-110'
                          : 'bg-gray-100 text-gray-600 hover:bg-[#ff6700] hover:text-white'
                      }`}
                    ...
                    {/* Image Box */}
                    <div className="w-20 h-20 md:w-28 md:h-28 mb-3 md:mb-4 mt-2 md:mt-3 flex items-center justify-center text-4xl md:text-6xl group-hover:scale-110 transition-transform duration-500 drop-shadow-lg flex-shrink-0">
                      {product.img}
                    </div>
                    ...
                    <div className="flex flex-col items-center w-full mb-3 md:mb-4">
                      ...
                      <p className="text-[#ff6700] font-bold text-[14px] md:text-[15px] tracking-tight">{product.price}</p>
                    </div>
